...
export function GameTile({ game }: { game: GameMeta }) {
  ...
  return (
    <Link
      ...
      className={clsx(
        'group relative flex h-48 w-72 shrink-0 flex-col justify-between overflow-hidden rounded-3xl border border-white/5 bg-white/5 p-6 text-left transition hover:border-white/40 hover:bg-white/10',
    ...
    >
      ...
      <div className="relative flex flex-col gap-2">
        <span className="text-xs uppercase tracking-[0.35em] text-slate-300">{game.status === 'available' ? 'Ready' : 'In Dev'}</span>
